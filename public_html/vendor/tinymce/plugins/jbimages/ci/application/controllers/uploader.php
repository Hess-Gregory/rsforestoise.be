...
class Uploader extends CI_Controller {
	...
	public function upload ($lang='english')
	{
		...
		if ($this->upload->do_upload()) // Success
		{
			...
			// Get dimensions resized
			// Must set correct image_width for each project
			if ($result['image_width'] > 1140) {
				$getImageProperties = $this->image_lib->get_image_properties('', TRUE);
				$result['final_size'] = $getImageProperties['size_str'];
			}
			else {
				$result['final_size'] = $result['image_size_str'];
			}
